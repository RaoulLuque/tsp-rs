use memchr::memchr;
use tsp_core::instance::{distance::Distance, matrix::TriangularMatrix, node::Node};
...
pub(super) fn parse_dists_from_full_matrix(
    ...
    println!("Parsed full matrix {:?}.", res_data);
...
pub(super) fn parse_dists_from_lower_row(
    ...
    loop_over_lines(
        ...
        &mut (&mut res_data, &mut current_row, &mut current_column),
        |line: &str, (res, current_row, current_column): &mut (&mut Vec<Distance>, &mut usize, &mut usize)| {
            parse_lower_triangular_line(res, line, current_row, current_column, diagonal_entry_present)
        },
        |(_, current_row, _): &(&mut Vec<Distance>, &mut usize, &mut usize)| {
            **current_row >= dimension
        },
...
pub(super) fn parse_dists_from_upper_row(
    ...
    let mut res = TriangularMatrix::new_from_dimension_with_value(dimension, Distance(0));
    let mut current_row = 0;
    let mut current_column = 0;
    ...
    loop_over_lines(
        ...
        |(_, current_row, _): &(&mut TriangularMatrix<Distance>, &mut usize, &mut usize)| {
...
fn parse_full_matrix_line(
    ...
    while start_index_in_line < line.len() {
        ...
        println!(
            "Parsing distance from '{}' at row {}, column {}",
            distance_str, *row, *column
        );
        // Given an explicit full matrix, we only store the distances for the lower triangular
        // indices
        if *column <= *row {
...
fn parse_upper_triangular_line(
    matrix_data: &mut TriangularMatrix<Distance>,
    line: &str,
    row: &mut usize,
    column: &mut usize,
    dimension: usize,
    ...
    while start_index_in_line < line.len() {
        ...
        matrix_data.set_data_to_bigger(Node(*row), Node(*column), distance);
